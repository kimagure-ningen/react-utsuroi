...
var VERSION2 = "4.0.310";

// ../media-parser/dist/esm/worker.mjs
...
var parseMediaOnWorkerImplementation = async ({ controller, reader, ...params }, worker, apiName) => {
  ...
  const onSeek = ({ detail: { seek: seek2 } }) => {
    post(worker, { type: "request-seek", payload: seek2 });
    controller?._internals.seekSignal.clearSeekIfStillSame(seek2);
  };
...
var __toESM2 = (mod, isNodeMode, target) => {
  ...
  for (let key4 of __getOwnPropNames(mod))
    if (!__hasOwnProp.call(to, key4))
      __defProp(to, key4, {
        get: () => mod[key4],
        enumerable: true
      });
...
var setRemotionImported = () => {
  ...
  globalThis.remotion_imported = VERSION2;
  if (typeof window !== "undefined") {
    window.remotion_imported = `${VERSION2}-webcodecs`;
  }
};
var maxClusterTimestamp = 2 ** 15;
var BIT_DEPTH = 16;
var BYTES_PER_SAMPLE = BIT_DEPTH / 8;
var import_licensing = __toESM2(require_dist(), 1);
var extractFrames = async ({
  fromSeconds,
  toSeconds,
  width: width2,
  height,
  ...
  console.log("extracting", fromSeconds, toSeconds, width2, height);
};
setRemotionImported();
...
import { VERSION as VERSION3 } from "remotion";
